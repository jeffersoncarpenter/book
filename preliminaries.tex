...
An example is the polymorphic identity function $\idfunc : \prd{A:\UU} A \to A$, which we define by $\idfunc{} \defeq \lam{A:\type}{x:A} x$.  (Not to be mistaken with the identity type $\mathsf{Id}_{A}$, which we discuss in \cref{sec:identity-types}.)
